...
Declare PtrSafe Function NWGetDefaultConnectionId Lib "Calwin32.dll" _
                                          Alias "NWGetDefaultConnectionID" (conn As Long) As Long
Declare PtrSafe Function NWGetConnectionStatus Lib "Calwin32.dll" _
                                       (ByVal conn_id As Long, nwinfo As nwinfo_struct, _
                                        ByVal xlen As Integer) As Long
...
251         If (gl_nwuser = "" Or Asc(Left(gl_nwuser, 1)) = 0) Then  'DM2023-05-12 Ein Stringvergleich mit dem Usernamen scheitert in 64 Bit, stattdessen den character code des ersten Zeichens auf 0 prüfen 'DM2021-10-06 Bei mir kommt als Ergebnis ein leerer Username (oder eine Reihe Sonderzeichen), wohl aufgrund der anderen Netzwerksituation
252             GoSub getUser_localuser
253         End If
