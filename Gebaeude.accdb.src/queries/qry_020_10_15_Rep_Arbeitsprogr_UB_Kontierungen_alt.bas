...
Begin OutputColumns
    ...
    Alias ="Kontierung"
    Expression ="[Finanzquelle] & \" / \" & [Finanzherkunft] & \" / \" & Format(IIf([Jahr]=Year(N"
        "ow())-1 And [HHR]>0,[HHR],IIf([Jahr]=Year(Now()) And [Ansatz]>0,[Ansatz],IIf([Ja"
        "hr]=Year(Now()) And [VE]>0,[VE],0))),\"#,##0.00 €\")"
End
Begin Joins
...
dbMemo "Filter" ="([qry_020_10_15_Rep_Arbeitsprogr_UB_Kontierungen].[ID]=1217)"
