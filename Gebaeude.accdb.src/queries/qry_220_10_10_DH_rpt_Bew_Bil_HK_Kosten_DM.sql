SELECT
  ...
  ) AS Herstellkosten_berechnet_alt,
  IIf(
  ...
  CDbl(
    HK_fuer_GebTeil_2022(
      [qry_rpt_200_10_10_Datenherk_Bewertungsbilanz].[ID_Gebäudeteil]
    )
  ) AS Herstellkosten_neu,
  qry_HK_Aus_B_Sanierungsstau.Herstellkosten
FROM
  qry_HK_Aus_B_Sanierungsstau
  INNER JOIN (
    (
      (
  ...
  ) ON qry_HK_Aus_B_Sanierungsstau.ID_Gebäudeteil = qry_rpt_200_10_10_Datenherk_Bewertungsbilanz.ID_Gebäudeteil
ORDER BY
